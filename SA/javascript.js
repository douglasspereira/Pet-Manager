...
let servicosgato = [
	{id: "500", servico: "Tosa", valor: "50,00"},
	{id: "501", servico: "Banho e Tosa", valor: "100,00"},
	{id: "502", servico: "Banho e Tosa Higiênica", valor: "70,00"}
				]

let servicoscachorro = [
	{id: "10", servico: "Tosa porte P", valor: "40,00"},
...
let funcionarios = [
	{nome: "Ricardo Scotti", email: "ricardo@petmanager.com", senha: "123"},
	{nome: "Douglas Pereira", email: "douglas@petmanager.com", senha: "456"},
	{nome: "Eliot Eloi", email: "eliot@petmanager.com", senha: "789"}
]

let cadastroclientes = [
	{Nome: "Mariela Silva", Email: "mariela@gmail.com", Telefone: "(48) 99986-3041"}
]
...
function cadastrarNovoFuncionario(){
	...
	for(let j = 0 ; j < funcionarios2.length ; j++)
		if(emailfuncionario == funcionarios2[j].email) {
			// alert("Este e-mail já é cadastrado");
			Swal.fire({
				position: 'top',
				icon: 'warning',
				iconColor: '#ff914d',
...
function cadastrarNovoCliente() {
	...
	if(flag !== 1){
		...
		else if (document.getElementById("error-email").innerHTML == "E-mail Inválido" || clienteemail == "") {
			Swal.fire({
				position: 'top',
				icon: 'warning',
				iconColor: '#ff914d',
...
document.getElementById("senhafuncionario1");
...
function efetuaAlteracaoCliente(){
	...
	for(let j = 0 ; j < cadastroclientes2.length ; j++)
		...
		if(document.getElementById("telefoneaalterar").value.length < 14){
	 	...
	 	if (document.getElementById("error-email2").innerHTML == "E-mail Inválido" || document.getElementById("emailaalterar").value == ""){
			// alert("Informe um Email válido")
			Swal.fire({
				position: 'top',
				icon: 'warning',
...
function efetuaAlteracaoFuncionario(){
	...
	for(let j = 0 ; j < funcionarios2.length ; j++)
		...
		if(document.getElementById("nomefuncionario").value.length < 3){
	 	...
	 	if (document.getElementById("error-email4").innerHTML == "E-mail Inválido" || document.getElementById("emailfuncionarioalterar").value == ""){
			// alert("Informe um Email válido")
			Swal.fire({
				position: 'top',
				icon: 'warning',
...
function imprimiServicos(){
  ...
  // tabela2.appendChild(linhacabecalho2)
	...
	for(let j = 0; j < servicoscachorro2.length; j++){
		...
		valor = criaCelula("td", formatter.format(parseInt(servicoscachorro2[j].valor)));
	...
	for(let k = 0; k < servicosgato2.length; k++){
		...
		valortabela = criaCelula("td", formatter.format(parseInt(servicosgato2[k].valor)));
...
function cadastrarNovoServico(){
	...
	switch (document.getElementById("gatooucachorroservico").selectedIndex){
		...
		Swal.fire({
			...
			icon: 'warning',
			iconColor: '#ff914d',
			title: 'Selecione se é gato ou cachorro',
			showConfirmButton: true,
			confirmButtonColor: '#836fff'
			// timer: 1500
		})
...
function deletarServico(){
	...
	if(servicoadeletar == "500"){
		...
		Swal.fire({
			icon: 'error',
			title: 'Este serviço não pode ser deletado, somente editado.',
			showConfirmButton: true,
			confirmButtonColor: '#836fff'
			// footer: '<a href>Porque estou com esse problema?</a>'
		})
		...
		for(let i = 0 ; i < servicosgato2.length ; i++){
			if(servicoadeletar == servicosgato2[i].id){
				...
				Swal.fire({
					...
					icon: 'success',
					iconColor: '#28a745',
					title: 'Serviço removido com sucesso (gato)',
					showConfirmButton: false,
					timer: 1500
				}).then((result) => {
					{
		...
		for(let j = 0 ; j < servicoscachorro2.length ; j++){
			if(servicoadeletar == servicoscachorro2[j].id){
				...
				Swal.fire({
					...
					icon: 'success',
					iconColor: '#28a745',
					title: 'Serviço removido com sucesso (cachorro)',
					showConfirmButton: false,
					timer: 1500
				}).then((result) => {
					{
...
function servicoGatoOuCachorro(){
	...
	switch (document.getElementById("gatooucachorrocotacao").selectedIndex){
		...
		document.getElementById("frasevalorunitario").textContent = `Valor unitário: R$ ${valordoservico}.`
...
function finalizaCotacao(){
	...
	if (datadacotacao == ""){
		...
		Swal.fire({
			...
			icon: 'warning',
			iconColor: '#ff914d',
			title: 'Agenda 2021 não está aberta',
			showConfirmButton: true,
			confirmButtonColor: '#836fff'
			// timer: 1500
		})
	...
	if(flag != 1 && bandera != 1 && globo != 1)
		if(document.getElementById("gatooucachorrocotacao").selectedIndex == 0){
			...
			Swal.fire({
				...
				icon: 'warning',
				iconColor: '#ff914d',
				title: 'Informe se é Gato ou Cachorro',
				showConfirmButton: true,
				confirmButtonColor: '#836fff'
				// timer: 1500
			})
	...
	if(flag != 1 && bandera != 1 && globo != 1 && saturno !=1)
	...
	if(flag !== 1 && bandera !== 1 && globo !== 1 && saturno !== 1 & plutao !== 1)
	...
	if(flag != 1 && bandera != 1 && globo != 1 && planeta != 1 && saturno !=1 && plutao !=1){
...
function salvaCotacao(){
	...
	if(clienteounao == ""){
		...
		Swal.fire({
			...
			icon: 'warning',
			iconColor: '#ff914d',
			title: 'Este cliente não é cadastrado',
			showConfirmButton: true,
			confirmButtonColor: '#836fff'
			// timer: 1500
		})
...
function ordenaData(a, b){
	var dataASplit = a.datadacotacao.split("/");
	var dataBSplit = b.datadacotacao.split("/");

	dataAformatada = `${dataASplit[1]}/${dataASplit[0]}/${dataASplit[2]}`
	dataBformatada = `${dataBSplit[1]}/${dataBSplit[0]}/${dataBSplit[2]}`

	return new Date (dataAformatada) - new Date (dataBformatada)
}
